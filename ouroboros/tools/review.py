...
EDGEE_CHAT_URL = "https://api.edgee.ai/v1/chat/completions"
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "models": {
                            ...
                            "items": {"type": "string"},
                            "description": (
                                "Edgee model identifiers to query "
...
async def _multi_model_review_async(content: str, prompt: str, models: list, ctx: ToolContext):
    ...
    api_key = os.environ.get("EDGEE_API_KEY", "") or os.environ.get("OPENROUTER_API_KEY", "")
    if not api_key:
        return {"error": "EDGEE_API_KEY not set"}
    ...
    try:
        ...
        # Finally check headers
            ...
            for key, value in headers_dict.items():
                if key.lower() in {"x-openrouter-cost", "x-edgee-cost"}:
                    cost = float(value)
                    break
